...
- Encryption key rotation endpoint: `POST /api/rotate-encryption-key` (protected by auth + CSRF)
...
Rotation-focused tests live under `internal/encryption`, `internal/database`, and `internal/api` (integration). Some tests require SQLCipher to be installed and available.
...
- Manual rotation via `POST /api/rotate-encryption-key` (authenticated + CSRF)
- Optional scheduler via `ENCRYPTION_KEY_ROTATION_INTERVAL` (e.g., `90d`, `720h`)
- Implementation uses SQLCipher `PRAGMA rekey` with pre-rotation filesystem backup and post-rotation verification
- Metadata tables track version and history:
  - `encryption_info (id=1, key_version, last_rotated_at)`
  - `encryption_rotation_log (from_version, to_version, rotated_at, backup_path, verification_ok, error)`

Failures leave the original key file unchanged and retain the backup. Success updates the key file atomically (0600 permissions) and increments `key_version`.
